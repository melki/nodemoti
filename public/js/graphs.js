...
var n = 1000,
  ...
  data[0] = new Array();
  data[1] = new Array();
  data[0][0] = d3.range(n).map(d3.random.normal(230, 0));
  data[0][1] = d3.range(n).map(d3.random.normal(10, 0));
  data[0][2] = d3.range(n).map(d3.random.normal(22, 0));
...
var letter = [["Z","X","Y"],["PITCH","YAW","ROLL"]];
...
svg[j].append("g")
 ...
 text[j] = svg[j].append("g").selectAll("text")
                        ...
                        .append("text")
                        .attr("x", 730)
                        .attr("y", function (d,i) { return ((data[j][i][998]*-1)+350)*270/700; })
                        .style("fill",function (d,i) {return color[i+1];})
                        .text(function (d,i) { return letter[j][i];});
  ...
  socket.on('serialEvent', function (socket_data)
   {
    ...
    data[0][0].push(socket_data.accel.z);
    data[0][1].push(socket_data.accel.y);
    data[0][2].push(socket_data.accel.x);
    data[1][0].push(socket_data.gyro.pitch);
    data[1][1].push(socket_data.gyro.yaw);
